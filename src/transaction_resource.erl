...
process_post(Request, Context) ->
    ...
    case proplists:get_value(do_redirect, Context, false) of
        false ->
            {{halt, 401}, wrq:set_resp_header(
                    "Content-type", "text/plain",
                    wrq:set_resp_body("NOT AUTHORIZED", Request)
                ), Context};
        Location ->
            {{halt, 302}, wrq:set_resp_header("Location", Location, Request), Context}
    end,
